...
static  int request_irqs(void)
{
    ...
    for (i = 0; i < MAX_COLUMN; i++) {
        ret = request_irq(key_info_matrix[i][0].irq_no, button_irq, SA_INTERRUPT,
                          BUTTONS_DEVICE_NAME, NULL);
...
/* 读键盘 */
static ssize_t button_read(struct file* filp, char* buffer, size_t count,
                           loff_t* f_pos)
{
    ssize_t ret = 0;
...
/* 清空键盘缓冲区 */
static int button_ioctl(struct inode* inode, struct file* file,
...
static void button_setup_cdev(struct cdev* dev, int minor,
                              struct file_operations* fops)
{
    ...
    dev->ops =
        fops; /* 给结构体里的ops成员赋初值，这里是对设备操作的具体的实现函数 */
    err = cdev_add(dev, devno,
                   1); /* 将结构提struct cdev添加到系统之中 */
...
static int  button_init(void)
{
    ...
    dev_t dev = MKDEV(button_major,
                      0); /* 将主设备号和次设备号定义到一个dev_t数据类型的结构体之中 */

    if (button_major) {
        ret = register_chrdev_region(dev, 1,
                                     BUTTONS_DEVICE_NAME);/* 静态注册一个设备，设备号先前指定好，并得到一个设备名，cat /proc/device来查看信息 */
    } else {
        ret = alloc_chrdev_region(&dev, 0, 1,
                                  BUTTONS_DEVICE_NAME);/* 如果主设备号被占用，则由系统提供一个主设备号给设备驱动程序 */
    ...
    button_setup_cdev(&button_dev, 0,
                      &button_fops); /* 初始化和添加结构体struct cdev到系统之中 */
...
static void __exit button_exit(void)
{
    ...
    unregister_chrdev_region(MKDEV(button_major, 0),
                             1); /* 卸载设备驱动所占有的资源 */
